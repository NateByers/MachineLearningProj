...
One way to select miRNAs is to use for predictors is to use a supervised method. So the outcome data (tumor stage) would be used to select the predictors. For each miRNA, we'll look at the expression value for each patient. For example, let's look at the first miRNA.
...
of each patient in the clinical data frame. Here's a box plot show the different distributions
